// A credential-free DNS/TLS smoke check using the same resolver as the gateway.
package main
...
func main() {
	failed := false
	for _, host := range []string{"cli-chat-proxy.grok.com", "auth.x.ai", "api.cline.bot", "models.dev"} {
		...
		fmt.Printf("%s DNS OK (%d addresses)\n", host, len(ips))
