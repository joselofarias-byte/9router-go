...
type Factors struct {
	...
	AccountRiskPenalty float64 // 0-30 points deducted for account-sensitive surfaces
}
...
func Calculate(f Factors) Score {
	...
	case trust.TrustTrusted:
		trustScore = 20.0
	case trust.TrustVerified:
		trustScore = 15.0
	case trust.TrustCandidate, trust.TrustUnknown:
		trustScore = 10.0
	case trust.TrustDegraded:
		trustScore = 5.0
	}
	...
	// 6. Account / product-surface risk penalty (max 30 points deducted).
	// This keeps risky surfaces available as fallbacks while preferring ordinary
	// API-key routes when quality, cost and health are otherwise comparable.
	riskPenalty := math.Max(0, math.Min(30, f.AccountRiskPenalty))
	score.Dimensions["account_risk_penalty"] = -riskPenalty

	total := trustScore + srScore + perfScore + costScore + benchScore - riskPenalty
