...
func TestWorkBuddyLooksLikeExpiredSession(t *testing.T) {
	if !workBuddyLooksLikeExpiredSession("Error: not logged in") {
		t.Fatal("expected expired session")
	}
	if workBuddyLooksLikeExpiredSession("model overloaded") {
		t.Fatal("quota text is not a session expiry")
	}
}
