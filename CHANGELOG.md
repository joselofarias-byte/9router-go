...
- Verifikasi silang vs instance upstream yang sedang jalan (port 20128, DB yang sama): **714 model upstream vs 738 9router-go, 688 id identik**; prefix, key set entry, dan key set `capabilities` sama persis. Breakdown: `clinepass` 469/469, `tr` 154/153, `kr` 34/44, `ag` 20/27, `cbai` 15/15, `openrouter` 7/10, `nvidia` 4/6, `Id` 3/3, `gcli` 1/4, combo 7/7.
- Sisa selisih 24 id dijelaskan oleh: (a) upstream memakai **live catalog** untuk kiro/grok-cli/qoder/github/clinepass (`kr/auto`, `gcli/grok-4.7`) sedangkan 9router-go memakai katalog statis; (b) data registry kita sedikit tertinggal (`ag` masih punya 7 model gemini-2.5 lama, `openrouter/free` belum ada); (c) model media yang id-nya tidak kena heuristik (`nvidia/fastpitch`, `tacotron2`, `openrouter/google/veo-3.1`, `openai/sora-2-pro`, `bytedance/seedance-2.0`) masih tampil — upstream menyaringnya lewat field `type` per model yang belum ada di registry Go.
